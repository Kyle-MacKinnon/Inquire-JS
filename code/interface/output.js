...
var output = {
   ...
   text(sentence) {
      ...
      moreWords.forEach(word => {
         this.words.push({ text: `<span id="word-${this.words.length}" class="text">${word} </span>`, index: this.words.length });
      });
      return this;
   },
   ...
   object(object, options = {}) {
      ...
      this.words.push({
         text: `<span id="word-${wordsLength}" class="object" onClick="output.focus('${wordsLength}')">${object.name}</span>${fullStop}${comma}`,
         object: object,
         index: wordsLength
      });
      return this;
   ...
   render() {
      ...
      this.words.forEach((word, index) => {
         ...
         word.lineIndex = wordElement.offsetTop;

         // Build an array of all words by line
         if (!this.lines[wordElement.offsetTop]) {
            this.lines[wordElement.offsetTop] = [];

            // Keep track of the highest line index
            this.maxLineIndex = wordElement.offsetTop;
         }
         this.lines[wordElement.offsetTop].push(word);
      });
      ...
      if (this.focusedWord) {
         ...
         this.words.forEach((word, index) => {
            ...
            if (this.focusedWord && wordToBreakOn && index == wordToBreakOn.index) {
               this.htmlElement.innerHTML += `${word.text} <div class="details" id="details"></div>`;
            } else {
               this.htmlElement.innerHTML += `${word.text} `;
            }
         });
   ...
   focusDetails() {
      let focusedWordElement = document.getElementById(`word-${this.focusedWord.index}`);
      const arrowImageStyle = `left: ${focusedWordElement.offsetLeft + focusedWordElement.offsetWidth / 2}px`;
      const arrowImage = `<img src="icons/small-arrow-up.png" class="focus-arrow" style="${arrowImageStyle}"></img>`;
      const name = `<div class="details-description">${toTitleCase(this.focusedWord.object.name)}</div>`;
